...
class Island():
    ...
    def nbOfPossibleNewBridge(self,direction):
        return min(self.bridgesRequired() - self.someOfBridge(),
...
class Hashiwokakera():
    ...
    def __createBridge__(self):
        ...
        possibleBridge = [i for i in possibleBridge if i[0] < nbOfLeftPossibleBridge + 1]
        possibleBridge = [i for i in possibleBridge if i[1] < nbOfUpPossibleBridge + 1]
        possibleBridge = [i for i in possibleBridge if i[2] < nbOfRightPossibleBridge + 1]
        possibleBridge = [i for i in possibleBridge if i[3] < nbOfDownPossibleBridge + 1]
        possibleBridge = [i for i in possibleBridge 
                          if sum(i) == self.graphe[self.cursor.getCurrentPosition()].bridgesRequired()
    ...
    def __foundNbOfPossibleBridge__(self,direction):
        ...
        while(type(self.graphe[cursor.getCurrentPosition()]) != Island 
              and (self.graphe[cursor.getCurrentPosition()] != -1
                   or self.graphe[self.cursor.getCurrentPosition()].nbOfBridge(direction) == 1)
        ...
        if self.graphe[cursor.getCurrentPosition()] == 0:
            return 0
        return self.graphe[cursor.getCurrentPosition()].nbOfPossibleNewBridge(Direction.opposite(direction))
...
graphe = np.array([[Island(4,2),-1,Island(4,2)],
                   [0,Island(0,0),Island(0,0)],
                   [Island(1,2),Island(0,0),Island(0,0)]], dtype = object)
